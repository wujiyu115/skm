...
describe('SkillCard', () => {
  ...
  it('shows Enabled badge when skill is enabled', () => {
    render(<SkillCard skill={mockSkill({ Enabled: true })} />)
    expect(screen.getByText('Enabled')).toBeInTheDocument()
  })

  it('shows Disabled badge when skill is disabled', () => {
    render(<SkillCard skill={mockSkill({ Enabled: false })} />)
    expect(screen.getByText('Disabled')).toBeInTheDocument()
  })

  it('shows source type badge', () => {
    render(<SkillCard skill={mockSkill({ SourceType: 'git' })} />)
  ...
  it('calls onRemove when remove clicked', async () => {
    ...
    render(<SkillCard skill={mockSkill()} onRemove={onRemove} />)
  ...
  it('calls onSync when sync clicked', async () => {
    const onSync = vi.fn()
    render(<SkillCard skill={mockSkill()} onSync={onSync} />)
  ...
  it('shows checkbox when onSelect provided', () => {
    render(<SkillCard skill={mockSkill()} onSelect={() => {}} selected={false} />)
    expect(screen.getByRole('checkbox')).toBeInTheDocument()
  })

  it('calls onSelect when checkbox clicked', async () => {
    const onSelect = vi.fn()
    render(<SkillCard skill={mockSkill()} onSelect={onSelect} selected={false} />)
